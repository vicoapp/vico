...
#include "AppleIncRootCertificate.h"
	u_int8_t		 ca_digest[SHA_DIGEST_LENGTH];
	SHA_CTX			 ca_ctx;
	u_int8_t		 bundle_fingerprint[SHA_DIGEST_LENGTH] = {
#include "bundle_fingerprint.h"
	};
	u_int8_t		 ca_fingerprint[SHA_DIGEST_LENGTH] = {
	...
	SHA1_Init(&ca_ctx);
	SHA1_Update(&ca_ctx, root_ca_xor, root_ca_len);
	SHA1_Final(ca_digest, &ca_ctx);
	if (bcmp(ca_digest, ca_fingerprint, SHA_DIGEST_LENGTH) != 0)
		_exit(173);
	...
	SHA1_Update(&guid_hash, opaque_buf, opaque_len);
	HEXDUMP(bundle_id, bundle_id_len, "adding %zu bytes of bundle id to guid hash", bundle_id_len);
	SHA1_Update(&guid_hash, bundle_id, bundle_id_len);

	HEXDUMP(bundle_ver, bundle_ver_len,
	    "adding %zu bytes of bundle version to bundle hash", bundle_ver_len);
	SHA1_Update(&bundle_ctx, bundle_ver, bundle_ver_len);

	SHA1_Final(ca_digest, &bundle_ctx); /* XXX: re-using ca_digest buffer */
	SHA1_Final(digest, &guid_hash);

	HEXDUMP(ca_digest, SHA_DIGEST_LENGTH, "computed bundle hash");
	HEXDUMP(bundle_fingerprint, SHA_DIGEST_LENGTH, "stored bundle hash");
	if (bcmp(ca_digest, bundle_fingerprint, SHA_DIGEST_LENGTH) != 0) {
		DPRINTF("bundle id/version fingerprint don't match");
		_exit(173);
	}

	HEXDUMP(digest, SHA_DIGEST_LENGTH, "computed guid hash");
	if (bcmp(digest, hash_buf, SHA_DIGEST_LENGTH) != 0) {
