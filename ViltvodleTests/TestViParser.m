...
{
	command = [parser pushKey:0x0E07 allowMacros:YES scope:nil timeout:nil error:&error];
...
- (void)test020_SimpleCommands
{
	...
	command = [parser pushKey:'i' allowMacros:YES scope:nil timeout:nil error:&error];
...
{
	command = [parser pushKey:'c' allowMacros:YES scope:nil timeout:nil error:&error];
...
{
	[parser pushKey:'c' allowMacros:YES scope:nil timeout:nil error:&error];
	command = [parser pushKey:'w' allowMacros:YES scope:nil timeout:nil error:&error];
...
{
	[parser pushKey:'c' allowMacros:YES scope:nil timeout:nil error:&error];
	command = [parser pushKey:'x' allowMacros:YES scope:nil timeout:nil error:&error];
...
{
	command = [parser pushKey:'3' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertNil(command, nil);
...
{
	command = [parser pushKey:'d' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertNil(command, nil);
	STAssertNil(error, nil);
	command = [parser pushKey:'3' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertNil(command, nil);
...
{
	command = [parser pushKey:'0' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertNotNil(command, nil);
	STAssertNil(error, nil);
...
{
	[parser pushKey:'9' allowMacros:YES scope:nil timeout:nil error:&error];
	[parser pushKey:'8' allowMacros:YES scope:nil timeout:nil error:&error];
	[parser pushKey:'7' allowMacros:YES scope:nil timeout:nil error:&error];
	[parser pushKey:'6' allowMacros:YES scope:nil timeout:nil error:&error];
	[parser pushKey:'5' allowMacros:YES scope:nil timeout:nil error:&error];
	[parser pushKey:'0' allowMacros:YES scope:nil timeout:nil error:&error];
	command = [parser pushKey:'x' allowMacros:YES scope:nil timeout:nil error:&error];
...
- (void)test064_DualCountIsMultiplicative
{
	/* From nvi:
         ...
         */
	[parser pushKey:'3' allowMacros:YES scope:nil timeout:nil error:&error];
	[parser pushKey:'y' allowMacros:YES scope:nil timeout:nil error:&error];
	[parser pushKey:'4' allowMacros:YES scope:nil timeout:nil error:&error];
	command = [parser pushKey:'w' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertNotNil(command, nil);
	STAssertEquals(command.count, 0, nil);
...
{
	command = [parser pushKey:'.' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertNil(command, nil);
	STAssertNotNil(error, nil);
...
{
	[parser pushKey:'x' allowMacros:YES scope:nil timeout:nil error:&error];
	command = [parser pushKey:'.' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertNotNil(command, nil);
...
- (void)test073_MotionDoesntResetDot
{
	...
	STAssertTrue(command.motion.hasOperator, nil);
	command = [parser pushKey:'j' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertNotNil(command, nil);
	STAssertFalse(command.fromDot, nil);
...
{
	[parser pushKey:'d' allowMacros:YES scope:nil timeout:nil error:&error];
	[parser pushKey:'2' allowMacros:YES scope:nil timeout:nil error:&error];
	[parser pushKey:'w' allowMacros:YES scope:nil timeout:nil error:&error];
	command = [parser pushKey:'.' allowMacros:YES scope:nil timeout:nil error:&error];
...
- (void)test078_GCommandSetsLineMode
{
	command = [parser pushKey:'d' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertFalse(command.isLineMode, nil);
	command = [parser pushKey:'G' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertTrue(command.isLineMode, nil);	
	STAssertEquals(command.count, 0, nil);
...
- (void)test078_gUUCommandSetsLineMode
{
	command = [parser pushKey:'g' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertFalse(command.isLineMode, nil);
	command = [parser pushKey:'U' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertFalse(command.isLineMode, nil);
	command = [parser pushKey:'U' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertTrue(command.isLineMode, nil);
...
{
	command = [parser pushKey:'t' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertNil(command, nil);
	STAssertNil(error, nil);
	command = [parser pushKey:'x' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertNotNil(command, nil);
...
{
	[parser pushKey:'t' allowMacros:YES scope:nil timeout:nil error:&error];
	NSString *euro = @"€";
	unichar ch = [euro characterAtIndex:0];
	command = [parser pushKey:ch allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertEquals(command.argument, ch, nil);
}

- (void)test090_DotCommandWithInsertedText
{
	command = [parser pushKey:'a' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertNotNil(command, nil);
	// input text
...
{
	[parser pushKey:'d' allowMacros:YES scope:nil timeout:nil error:&error];
	[parser pushKey:'f' allowMacros:YES scope:nil timeout:nil error:&error];
	command = [parser pushKey:'a' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertEquals(command.argument, (unichar)0, nil);
...
{
	command = [parser pushKey:0x17 allowMacros:YES scope:nil timeout:nil error:&error];	// C-w
	STAssertNil(command, nil);
	STAssertNil(error, nil);
	command = [parser pushKey:'l' allowMacros:YES scope:nil timeout:nil error:&error];
	STAssertNotNil(command, nil);
...
{
	[parser pushKey:'d' allowMacros:YES scope:nil timeout:nil error:&error];
	[parser pushKey:'1' allowMacros:YES scope:nil timeout:nil error:&error];
	[parser pushKey:'0' allowMacros:YES scope:nil timeout:nil error:&error];
	command = [parser pushKey:'g' allowMacros:YES scope:nil timeout:nil error:&error];
